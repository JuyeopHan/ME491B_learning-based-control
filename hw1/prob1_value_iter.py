...
def get_optim_value(D=None, threshold=0.001, gamma=0.9, depart_pos=7, terminal_pos=0):
    ...
    num_nodes = len(D)
    for i in range(num_nodes):
        optim_value.append(0)
        former_value.append(-1)
    optim_value[terminal_pos] = 0
    #Dynamic Programming
    while(former_value != optim_value):
        former_value = copy.deepcopy(optim_value)
        for i in range(num_nodes):

            max_val = MIN

            for j in range(num_nodes):
                if (D[i][j] != 0 ) and (- D[i][j] + gamma*former_value[j] > max_val):
                    max_val = - D[i][j] + gamma*former_value[j]

            optim_value[i] = max_val
            optim_value[terminal_pos] = 0
